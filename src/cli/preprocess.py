...
from src.utils.data_io import load_raw_data, save_processed_data
...
@click.option(
    "--min-user-ratings", type=int, default=5, help="Minimum number of ratings per user"
)
@click.option(
    "--min-movie-ratings",
    type=int,
...
@click.option(
    "--min-tags-per-item", type=int, default=3, help="Minimum number of tags per item"
)
@click.option(
    "--log-level",
...
@click.option(
    "--log-file", type=click.Path(), default=None, help="Optional log file path"
)
def preprocess(
    data_dir: str,
    output_dir: str,
    ...
    try:
        ...
        ratings_df, movies_df, tags_df = preprocessor.preprocess(
        ...
        if tags_df is not None:
            user_tags_dict, movie_tags_dict = preprocessor.create_tag_dictionaries(
                tags_df
            )
        ...
        plot_rating_distribution(
            ratings_df["rating"], save_path=output_path / "rating_distribution.png"
        )
