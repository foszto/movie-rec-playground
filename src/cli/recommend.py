...
@click.option(
    "--user-id", type=int, required=True, help="User ID to generate recommendations for"
)
@click.option(
    "--n-recommendations",
    type=int,
...
def recommend(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Could not find required data files in {data_dir}. Please check if the data directory contains preprocessed files."
            )
            raise click.UsageError(
                f"Data files not found in directory: {data_dir}"
            ) from e
        ...
        if not candidate_movies:
            logger.warning(
                f"No {'unwatched ' if not include_watched else ''}movies available for recommendations!"
            )
            return
        ...
        predictions = asyncio.run(
            model.predict(user_ids, item_ids, movie_info_dict, user_history_dict)
        )
        ...
        for movie_id, score in movie_scores[:n_recommendations]:
            ...
            if movie_id in watched_movies:
                previous_rating = user_history.loc[
                    user_history["movieId"] == movie_id, "rating"
                ].iloc[0]
        ...
        recommended_movies = set(
            movie_id for movie_id, _ in movie_scores[:n_recommendations]
        )
        new_recommendations = recommended_movies - watched_movies
        print("\nRecommendation statistics:")
        print(f"- New movies: {len(new_recommendations)}/{n_recommendations}")
        print(
            f"- Previously rated: {n_recommendations - len(new_recommendations)}/{n_recommendations}"
        )
