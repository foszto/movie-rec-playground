...
class LLMFeatureExtractor:
    ...
    async def process_user_tags(self, user_tags: pd.DataFrame) -> torch.Tensor:
        ...
        weights = []
        for tag in top_tags:
            embedding = self.model.encode(tag, convert_to_tensor=True).to(
                dtype=torch.float32
            )
    ...
    async def process_movie_tags(self, movie_tags: pd.DataFrame) -> torch.Tensor:
        ...
        movie_tags.loc[
            movie_tags["timestamp"] > movie_tags["timestamp"].median(), "weight"
        ] = 1.2
        ...
        tag_tensor = torch.stack(tag_embeddings)
        tag_importance = torch.softmax(
            torch.tensor(tag_weights[: len(tag_embeddings)]), dim=0
        )
    ...
    async def get_user_preference_embedding(
        self, user_history: pd.DataFrame, user_tags: Optional[pd.DataFrame] = None
    ) -> torch.Tensor:
    ...
    async def _get_base_user_embedding(
        ...
        for genre, score in top_genres:
            ...
            if not genre_movies.empty:
                ...
                similar_text = (
                    f" (similar to {', '.join(g['genre'] for g in similar_genres[:2])})"
                    if similar_genres
                    else ""
                )
        ...
        user_history["time_weight"] = user_history["timestamp"].apply(
            lambda x: float(
                np.exp(-0.1 * (max_timestamp - x) / (24 * 60 * 60))
            )  # Decay over days
        )
        ...
        try:
            with torch.no_grad():
                ...
                embedding = self.model.encode(profile_text, convert_to_tensor=True).to(
                    dtype=torch.float32
                )
    ...
    async def get_movie_embedding(
        self, movie_info: Dict, movie_tags: Optional[pd.DataFrame] = None
    ) -> torch.Tensor:
