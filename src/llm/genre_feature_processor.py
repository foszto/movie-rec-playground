...
class OptimizedGenreProcessor:
    ...
    def _vectorize_genre_stats(self, user_history: pd.DataFrame) -> Dict:
        ...
        genre_df['time_weight'] = np.exp(-0.1 * (max_time - genre_df['timestamp']) / (24 * 60 * 60))
        
        # Calculate weighted stats
        weighted_stats = genre_df.groupby('genre').agg({
            'rating': lambda x: np.average(x, weights=genre_df.loc[x.index, 'time_weight']),
            'time_weight': 'sum'
        })
    ...
    def _fast_similarity_calculation(self, user_history: pd.DataFrame) -> Tuple[Dict, Set]:
        ...
        for g1 in all_genres:
            ...
            similarities[g1] = sorted(similarities[g1], key=lambda x: x[1], reverse=True)[:3]
    ...
    def generate_genre_feature_text(self, preferences: Dict) -> str:
        ...
        if preferences['liked_genres']:
            ...
            for genre in top_genres:
                if genre in preferences['similar_genres'] and preferences['similar_genres'][genre]:
        ...
        if preferences['disliked_genres']:
            ...
            scores = [f"{g} ({preferences['genre_scores'][g]:.2f})" for g in bottom_genres]
